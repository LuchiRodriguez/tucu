    ...
    <StyledExerciseItem>
      <Card
        ...
        onToggleActiveOrCompleted={() =>
          onToggleCompletion(routineId, exercise.id)
        } // Pasamos el ID de la rutina y el ejercicio
        ...
        onKilosChange={(kilosValue) =>
          onKilosChange(routineId, exercise.id, kilosValue)
        } // Pasamos los IDs y el nuevo valor
        ...
        <StyledExerciseDetails>
          ...
          {exercise.type === "timed" ? (
            <StyledExerciseDetailText className="Exercise-sets-reps">
              Series: <span>{exercise.sets}</span> | Tiempo:{" "}
              <span>{exercise.time}s</span>
            </StyledExerciseDetailText>
          ) : (
            <StyledExerciseDetailText className="Exercise-sets-reps">
              Series: <span>{exercise.sets}</span> | Repeticiones:{" "}
...
Exercise.propTypes = {
  ...
  routineId: PropTypes.oneOfType([PropTypes.string, PropTypes.number])
    .isRequired, // ID de la rutina padre
