...
const ExerciseList = ({ onAddExerciseToRoutine }) => {
  const { exercises, loading, error, searchExercises } = useExercisesApi();
    ...
    <StyledExerciseListContainer>
      ...
      {error && (
        <ErrorMessage isVisible={true}>
          ¡Uups! Hubo un error al buscar ejercicios. Por favor, intentá de
          nuevo.
        </ErrorMessage>
      )}
      ...
      {!loading &&
        ...
        searchTerm.length <= 2 && (
          <StyledExerciseListMessage>
            Escribí al menos 3 letras para buscar ejercicios.
          </StyledExerciseListMessage>
        )}
            ...
            <StyledExerciseListItem key={exercise.id}>
              ...
              >
                <StyledExerciseSearchDetails>
                  ...
                  {exercise.muscles_names &&
                    exercise.muscles_names.length > 0 && (
                      <StyledExerciseSearchDetailText className="Exercise-muscles">
                        Músculos:{" "}
                        <span>{exercise.muscles_names.join(", ")}</span>
                      </StyledExerciseSearchDetailText>
                    )}
                  {exercise.equipment_names &&
                    exercise.equipment_names.length > 0 && (
                      <StyledExerciseSearchDetailText className="Exercise-equipment">
                        Equipo:{" "}
                        <span>{exercise.equipment_names.join(", ")}</span>
                      </StyledExerciseSearchDetailText>
                    )}
                </StyledExerciseSearchDetails>
              </Card>
