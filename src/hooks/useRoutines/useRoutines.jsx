...
const useRoutines = () => {
  ...
  const updateRoutineExercises = async (routineId, updatedExercises) => {
    try {
      const routineRef = doc(db, "routines", routineId);
      // Actualizamos el campo exerciseList con el nuevo orden
      await updateDoc(routineRef, { exerciseList: updatedExercises });
      console.log("Ejercicios actualizados correctamente.");
    } catch (err) {
      console.error("Error al actualizar el orden de ejercicios:", err);
    }
  };
